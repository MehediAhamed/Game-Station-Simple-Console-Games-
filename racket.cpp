...
//for looping the racket functions
...
void input_racket(char cell[r_border_vertical][r_border_horizon], int *p_ini, int *p_final, int *cpuini, int *cpufinal, int *px, int *py, int *modx, int *mody, int *modia, int *goal)
{
	...
	//ball logic for changing direction when about to touch the border

	if (*py == 1 || *py == r_border_vertical - 2)
	{
	...
	// after crossing boundary the while loop_racket stops
	if (*px == 1)
	{
		// Verify GOL
   ...
   // ball_racket passed computer side

	if (*px == r_border_horizon - 2)
	{
		player_racketwin = 1;
		*goal = 1; // game stops
	}
	...
	// logic for computer racket movement
